...
from pism_terra.download import download_netcdf
...
def _process_single_forcing(
    ...
    for var in ds.data_vars:
        if var not in ("spatial_ref", "time", "time_bnds"):
            encoding[var] = {"zlib": True, "complevel": 2, "_FillValue": None}
...
def prepare_forcing(
    ...
    cdo.debug = True
    processed_files = []
    for i_file in o_files:
        i_file = Path(i_file)
        name = i_file.name
        o_file = output_path / Path(name)
        cdo.settbounds(
            "1mon",
            input=f"""-setreftime,1850-01-01 {i_file}""",
            output=str(o_file),
            options="-O -f nc4 -z zip_2",
        )
        i_file.unlink()
        check_xr_lazy(o_file)
