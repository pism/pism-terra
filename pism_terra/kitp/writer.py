...
class OutputGrid(object):
    ...
    def __init__(self):
        raise NotImplementedError("Cannot create an instance of OutputGrid: use a derived class")
    ...
    def mapping(self):
        return self.mapping_attributes
    ...
    def lonlat_corner(self):
        Mx, My = self.size()

        dx = self.dx()
        x_corner = np.linspace(self.x[0] - 0.5 * dx, self.x[-1] + 0.5 * dx, Mx + 1)

        dy = self.dy()
        y_corner = np.linspace(self.y[0] - 0.5 * dy, self.y[-1] + 0.5 * dy, My + 1)

        xx, yy = np.meshgrid(x_corner, y_corner)
    ...
    See https://theghub.org/groups/ismip6/wiki/ISMIP6-Projections-Greenland

    """

    def __init__(self, resolution=1000):
        self.proj_string = "EPSG:3413"
        x_min = -720e3
        x_max = 960e3
        y_min = -3450e3
        y_max = -570e3
...
class ISMIP7GreenlandGrid(OutputGrid):
    """Grid defined in 'Appendix 1 – Output grid definition and interpolation' of 'ISMIP6
    Projections Greenland'
...
class StdGreenlandGrid(OutputGrid):
    """Grid used in Bamber et al., 2001 and by PISM's "std-greenland" example"""
...
class YacWrapper:
    ...
    def __init__(self):
        ...
        self.intercomm = local_comm.Create_intercomm(0, global_comm, self.remote_leader, tag=0)
    ...
    def receive_string(self, comm):
        ...
        try:
            ...
            result = string.decode("ascii", errors="replace")
            logger.critical(f"failed to decode '{result}'")
            raise

    def receive_array(self, comm):
        status = MPI.Status()
        comm.Probe(source=self.remote_leader, tag=0, status=status)

        length = status.Get_count(MPI.DOUBLE)

        array = np.empty(length, dtype="d")
        comm.Recv([array, MPI.DOUBLE], source=self.remote_leader, tag=0)

        return array

    def fake_lon_lat(self, x, y):
        x_min = x[0]
        y_min = y[0]
        ...
        for r in range(My):
            for c in range(Mx):
                lon[r, c] = (x[c] - x_min) / x_span
                lat[r, c] = (y[r] - y_min) / y_span

        return lon, lat

    def receive_grid_info(self):
        proj_string = self.receive_string(self.intercomm)
        x = self.receive_array(self.intercomm)
        y = self.receive_array(self.intercomm)

        return proj_string, x, y

    def define_matching_grid(self, grid_name, proj_string, x, y):

        if proj_string == "":
            logger.debug(f"grid {grid_name} projection: none")
            lon, lat = self.fake_lon_lat(x, y)
        else:
        ...
        return grid, interpolation_stack
    ...
    def define_yac_grid(self, metadata, output_grid=None):
        ...
        #
        # Also sets grid.points for self.grids[grid_name]
        # the grid name in pism_output has to be different
        grid_name = metadata["grid_name"]

        proj_string, x, y = self.receive_grid_info()

        if output_grid is None:
            logger.debug(f"grid size: {(len(x), len(y))}")
            x_size = len(x)
            y_size = len(y)
            ...
            grid, interpolation_stack = self.define_matching_grid(grid_name, proj_string, x, y)

            self.grids[grid_name] = grid
            self.output_grid_size[grid_name] = (x_size, y_size)
            self.interpolation_stack[grid_name] = interpolation_stack
            self.vertex_indexes[grid_name] = np.arange(x_size * y_size)
            return
        ...
        grid = yac.Curve2dGrid(self._output_grid_name(grid_name), lon_corner, lat_corner)
        # and points using cell centers:
        lon, lat = output_grid.lonlat_center()
        grid.points = grid.def_points(yac.Location.CELL, lon, lat)

        interpolation_stack = yac.InterpolationStack()
        # Use conservative interpolation from PISM's grid plus nearest neighbor as backup:
        interpolation_stack.add_conservative(
            order=1, enforced_conserv=True, partial_coverage=False, normalisation=yac.ConservNormalizationType.DESTAREA
        )
        interpolation_stack.add_nnn(reduction_type=yac.NNNReductionType.AVG, n=1)

        Mx, My = output_grid.size()
        self.grids[grid_name] = grid
        self.output_grid_size[grid_name] = (Mx, My)
        self.vertex_indexes[grid_name] = np.arange(Mx * My)
        self.interpolation_stack[grid_name] = interpolation_stack
    ...
    def define_field(self, metadata):
        ...
        points = self.grids[grid_name].points
        self.fields[field_name] = yac.Field.create(
            field_name, self.component, points, collection_size, timestep, yac.TimeUnit.ISO_FORMAT
        )

        self.yac.def_couple(
            self.source_comp_name,
            grid_name,
            field_name,
            self.target_comp_name,
            self._output_grid_name(grid_name),
            field_name,
            timestep,
            time_unit,
            time_reduction,
    ...
    def receive_action(self):
        ...
        string = self.receive_string(self.intercomm)
    ...
    def receive_gridded_variable(self, metadata):
        ...
        result = np.ndarray(shape=(collection_size, y_size, x_size), buffer=data, dtype="f8")
...
class OutputFile:
    ...
    def define_variable(self, metadata):
        ...
        nc_variable = self.nc_dataset.createVariable(
            name,
            metadata["dtype"],
            dimensions,
    ...
    def write_gridded_variable(self, array, metadata):
        ...
        if ndims == 2:
            ...
            tmp = np.ndarray(shape=(y_size, x_size, collection_size), dtype=array.dtype)
    ...
    while True:
        ...
        match action_id:
            ...
            case ServerActions.OPEN_FILE.value:
                ...
                if one_var_per_file(file_name):
                    logger.debug(f"OPEN_FILE {file_name}: cannot append to a file split across variables")
                else:
