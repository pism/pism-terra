...
def scalar_file(path: Path, labels=(GLACIER_A, GLACIER_B), values=(1.0, 2.0), n_time: int = 3) -> Path:
    """
...
def test_cli_end_to_end(tmp_path, release, rgi, monkeypatch):
    ...
    assert {"Ba", "Ba_mwe", "Bw_unc", "tendency_of_ice_mass", "area"} <= set(out.data_vars)
