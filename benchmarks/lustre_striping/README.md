...
Not yet measured. Fill this in from `results.csv` when the matrix has run,
then set the winning layout in `pism_terra/templates/chinook-apptainer.j2`
and `chinook-apptainer-async.j2` and note the date and the PISM version here
— the answer depends on both the filesystem's state and on how PISM writes.
